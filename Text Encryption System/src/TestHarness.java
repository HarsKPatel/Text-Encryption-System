...
public class TestHarness {
    ...
    private static final String caesartextCommand = "caesarcipher";
    private static final String inferCipherCommand = "infershift";
    ...
    private static void printUsage() {
        ...
        System.out.println("  " + caesartextCommand + " <string>");
        System.out.println("  " + inferCipherCommand + " <string>");
    ...
    private static void printKey( ISubstitutionCipher cipher ) {
        System.out.println( "System reports key valid status:\n" + cipher.getEncryptionKey().keyIsValid());
        ...
        // to have around.
            ...
            do {
                ...
                if (userCommand.equalsIgnoreCase(plaintextCommand)) {
                    ...
                    Boolean added = substitutionCipher.plaintext( text );
                    ...
                    String text = getEndingString( userInput );
                    Boolean added = substitutionCipher.ciphertext( text );
                    ...
                    Boolean added = substitutionCipher.plaintext( storedPlaintext );
                    ...
                    Boolean added = substitutionCipher.ciphertext( storedCiphertext );
                    ...
                    System.out.println( "System reports key valid status:\n" + substitutionCipher.getEncryptionKey().keyIsValid());

                    if (!substitutionCipher.encodeText()) {
                        ...
                        if (screen != null) {
                            substitutionCipher.extractCiphertext(screen);
                            screen.flush();
                        }
                        else
                        {
                    ...
                    if (!substitutionCipher.decodeText()) {
                        ...
                        if (screen != null) {
                            substitutionCipher.extractPlaintext(screen);
                            screen.flush();
                        }
                        else
                        {
                    ...
                    Boolean changed = substitutionCipher.setDecodeLetter( plainChar, cipherChar );

                    System.out.println( "Change return:\n" + changed );
                    System.out.println( "System reports key valid status:\n" + substitutionCipher.getEncryptionKey().keyIsValid());
                }
                else if (userCommand.equalsIgnoreCase(caesartextCommand)) {
                    int shiftValue = userInput.nextInt();
                    ignoredString = getEndingString( userInput );
                    Boolean added = substitutionCipher.setCaesarCipher( shiftValue );

                    if (added == null) {
                        System.out.println("There was an unknown error while setting caesar cipher");
                    }
                    else if (!added) {
                        System.out.println("Shift value not in appropriate range.");
                    }
                    else {
                        printKey(substitutionCipher);
                    }
                }
                else if (userCommand.equalsIgnoreCase(inferCipherCommand)) {
                    ignoredString = getEndingString( userInput );
                    int inferredShift = ((ISubstitutionKey)substitutionCipher.getEncryptionKey()).inferShift();

                    System.out.println("Most likely shift is " + inferredShift);
                }
