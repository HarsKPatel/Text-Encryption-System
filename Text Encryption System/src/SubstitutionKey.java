 ...
 */
public class SubstitutionKey implements X {
     ...
     */
    public ISubstitutionKey invertKey() {
    ...
    public int inferShift() {
        final int[] inferredShift = {0};
        final int[] maxValuedShift = {0};

        HashMap<Integer, Integer> shifts = new HashMap<>();
        key.forEach((k, v) -> {
            if (v - k < 26 && shifts.containsKey(v - k)) {
                shifts.replace(v - k, shifts.get(v - k) + 1);
            }
            else if (v - k < 26 && !shifts.containsKey(v - k)) {
                shifts.put(v - k, 1);
            }
            else if (v - k >= 26 && shifts.containsKey(v - k - 26)) {
                shifts.replace(v - k - 26, shifts.get(v - k - 26) + 1);
            }
            else if (v - k >= 26 && !shifts.containsKey(v - k - 26)){
                shifts.put(v - k - 26, 1);
            }
        ...
        shifts.forEach((k, v) -> {
            if (v > maxValuedShift[0]) {
                maxValuedShift[0] = v;
                inferredShift[0] = k;
            }
        });

        return inferredShift[0];
    }
    ...
    public Boolean keyIsValid() {
        ...
        HashSet<Character> valuesUsed = new HashSet<>(key.values());
        Boolean valid = key.keySet().size() == valuesUsed.size();
